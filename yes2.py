...
def fetch_teamrankings_data():
    ...
    # Debugging: Check if page content is loaded
    st.write("TeamRankings Page HTML:", response.text[:1000])
    
    table = soup.find("table", class_="tr-table datatable scrollable")
    if not table:
        st.error("Error: Could not find TeamRankings data table. Structure might have changed.")
        return pd.DataFrame(columns=["Team", "AdjO", "AdjD", "Tempo", "SOS"])
    ...
    if not data:
        st.error("Error: No team data found. Check if TeamRankings changed their structure.")
...
def fetch_sportsbook_odds():
    ...
    st.write("Sportsbook API Response:", data)
